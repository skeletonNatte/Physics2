...
class Box{
    constructor(pos, height, width, ang = 0, isStatic = false, elasticity = 0.25, m = 1){
        ...
        this.static = isStatic;
        this.elast = elasticity;
        this.mass = m//(this.width * this.height);
        if(this.mass == 0 || this.static){
            this.invM = 0;
        } else {
            this.invM = 1/this.mass;
...
function loadPreset1(){
    ...
    createWall(CANVAS_WIDTH,-400,CANVAS_WIDTH,CANVAS_HEIGHT);
    createWall(0,CANVAS_HEIGHT,0,-400);
...
function loadPreset2(){
    ...
    for(let i = 0; i < 20; i++){
        createBox(
            ...
            1,
            undefined,
            new Vector(random(-25,25),random(-25,25))
        );
        createBall(
            random(37.5,CANVAS_WIDTH-37.5),
            random(37.5,CANVAS_HEIGHT-37.5),
...
function loadPreset3(){
    ...
    airRes = 0.0000025;
    gravity = 21;
    createSoft(500,-250,floor(random(3,9),0),floor(random(4,18),0),30);
    ...
    for(let i = 0; i < 5; i++){
        createBall(random(17.5,CANVAS_WIDTH-17.5),CANVAS_HEIGHT,random(35,100),true);
    }
}
...
function loadPreset4(){
    ...
    let pos1 = new Vector(random(-1,1),random(-1,1)).unit().mult(150).add(mp.pos);
    let pos2 = new Vector(random(-1,1),random(-1,1)).unit().mult(150).add(pos1);
    createBall(pos1.x,pos1.y,2);
    createBall(pos2.x,pos2.y,2);
    attatch(mp,OBJECTS[0],150);
    attatch(OBJECTS[0],OBJECTS[1],150);
    /*OBJECTS.sort(function(a, b){
        return projectOnToAxis(new Vector(1,0),a).min - projectOnToAxis(new Vector(1,0),b).min;
    });*/
}
...
function testPreset(){
    ...
    airRes = 0;
    gravity = 0;
    createBox(CANVAS_WIDTH/2,CANVAS_HEIGHT/2,100,100);
}
                ...
                if(collidingOverAxis(active[j],OBJECTS[i],axis)){
                    if(collidingOverAxis(active[j],OBJECTS[i],axis.normal())){
                        ...
                        if(currSat){
                            ...
                            penRes(active[j], OBJECTS[i], collData);
                            collRes(active[j], OBJECTS[i], collData);
                        }
                    }
                } else {
                    active.splice(j,1);
...
function render(){
    ...
    OBJECTS.forEach(obj => {
        obj.draw();
        //obj.vel.draw(obj.pos.x,obj.pos.y,1,"rgb(165,0,0)");
    });
    JOINED.forEach(pair => {
